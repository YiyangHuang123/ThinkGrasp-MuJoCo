...
class GraspNetBaseLine():
    ...
    def inference(self, o3d_pcd):
        ...
        print(
            "Raw GraspNet network proposals:",
            len(gg),
        )

        if self.collision_thresh > 0:
            gg = self.collision_detection(
                gg,
                np.array(cloud.points),
            )
            print(
                "After internal GraspNet collision detection:",
                len(gg),
            )
        else:
            print(
                "After internal GraspNet collision detection: skipped"
            )
